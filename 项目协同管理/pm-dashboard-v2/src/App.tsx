...
const STRATEGY_OPTIONS = [
    { label: '现有产品改造', value: 'customization', color: 'purple' },
    { label: '重新定制开发', value: 'development', color: 'blue' },
    { label: '高保真DEMO', value: 'demo', color: 'orange' },
    { label: '文字说明响应', value: 'text_description', color: 'green' }
];
...
const ListView = ({ modules, stats, filterMode, onFilterChange, onSelect, onAdd, onImport, onExport, onDelete, onDeleteAll, onReorderModules, onReorderSubsystems, searchText, onSearchChange, userRole, onLogout, onOpenProjectInfo, onOpenWeeklyReports, projectInfo }) => {
    ...
    const tableColumns = [
        ...
        { title: '所属子系统', dataIndex: 'subsystem', key: 'subsystem', width: 180, render: (text) => <Tag className="border-none bg-slate-100">{text}</Tag> },
        { title: '响应策略', dataIndex: 'strategyType', key: 'strategyType', width: 150, render: (val) => { const opt = STRATEGY_OPTIONS.find(o => o.value === val); return opt ? <Tag color={opt.color} className="border-none">{opt.label}</Tag> : '-'; } },
        { title: '进度', dataIndex: 'progress', key: 'progress', width: 180, sorter: (a, b) => (a.progress || 0) - (b.progress || 0), render: (val) => <Progress percent={val || 0} size="small" strokeColor="#3b82f6" /> },
        { title: '预计完成时间', dataIndex: 'deadline', key: 'deadline', width: 150, sorter: (a, b) => dayjs(a.deadline).unix() - dayjs(b.deadline).unix(), render: (text) => <span className="font-mono text-slate-500">{text || '-'}</span> },
        {
            title: '状态',
            key: 'delayed',
            width: 120,
    ...
    return (
        <div className="max-w-7xl mx-auto p-6 fade-in">
            <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <Space className="ml-4">
                        ...
                        <Button icon={<AuditOutlined />} onClick={onOpenWeeklyReports} className="rounded-full border-indigo-200 text-indigo-600 hover:bg-indigo-50 font-medium">项目周报</Button>
                    </Space>
                </div>
            ...
            <Row gutter={12} className="mb-8">
                <Col span={4}><Card size="small" className={`h-36 flex flex-col justify-center clickable-card ${filterMode === 'all' ? 'stat-card-active' : ''}`} onClick={() => onFilterChange('all')}><div className="text-slate-500 text-xs mb-3 font-bold">总体进度</div><div className="text-center"><span className="text-blue-600 text-2xl font-bold">{stats.totalProgress}%</span></div><Progress percent={stats.totalProgress} showInfo={false} strokeWidth={10} /><div className="text-slate-400 text-[10px] mt-3">共 <span className="text-slate-800 font-bold">{stats.total}</span> 模块</div></Card></Col>

                <Col span={6}>
                    <Card size="small" className="h-36">
                        <div className="text-slate-500 text-xs mb-3 font-bold">响应策略</div>
                ...
                <Col span={4}>
                    <Card size="small" className={`h-36 clickable-card ${filterMode === 'completed' ? 'stat-card-active' : ''}`} onClick={() => onFilterChange('completed')}>
                        <div className="text-slate-500 text-sm mb-1 font-bold">研发进展</div>
                        ...
                        <div className="text-blue-600 text-[10px] font-bold mb-2">{stats.completedSubsystems} <span className="font-normal text-slate-400">/ {stats.subsystemCount} 子系统完成</span></div>
                        <div className="inline-flex items-center px-3 py-1 bg-green-50 text-green-600 rounded-full border border-green-100 text-[9px] font-bold"><SafetyOutlined className="mr-1" /> 已完成明细</div>
                    </Card>
                </Col>
                <Col span={4}><Card size="small" className={`h-36 clickable-card ${filterMode === 'delayed' ? 'stat-card-active' : ''}`} onClick={() => onFilterChange('delayed')}><div className="text-slate-500 text-xs mb-1 font-bold">进度延误</div><div className="text-4xl font-bold text-red-500 mb-2">{stats.delayedCount} <span className="text-xs font-normal text-slate-400">项</span></div><div className="inline-flex items-center px-2 py-0.5 bg-red-50 text-red-600 rounded-full border border-red-100 text-[9px] font-bold"><AlertOutlined className="mr-1" /> 风险预警</div></Card></Col>
                <Col span={3}><Card size="small" className={`h-36 clickable-card ${filterMode === 'pendingIssues' ? 'stat-card-active' : ''}`} onClick={() => onFilterChange('pendingIssues')}><div className="text-slate-500 text-xs mb-1 font-bold">协调事项</div><div className="text-4xl font-bold text-slate-800 mb-2">{stats.pendingIssues} <span className="text-xs font-normal text-slate-400">项</span></div><div className="inline-flex items-center px-2 py-0.5 bg-orange-50 text-orange-600 rounded-full border border-orange-100 text-[9px] font-bold"><ClockCircleOutlined className="mr-1" /> 需配合</div></Card></Col>
                <Col span={3}>
                        ...
                        return (
                            <div key={ssName} onDragOver={(e) => { e.preventDefault(); setDragOverSubsystem(ssName); }} onDragLeave={() => setDragOverSubsystem(null)} onDrop={(e) => handleSubsystemDrop(e, ssName)} className={`fade-in p-2 rounded-xl transition-all ${dragOverSubsystem === ssName ? 'subsystem-drag-over' : ''}`}>
                                <div className="subsystem-header shadow-sm group" draggable={userRole === 'team'} onDragStart={(e) => handleSubsystemDragStart(e, ssName)} onClick={() => toggleCollapse(ssName)}>
                                    ...
                                    <Tag className="bg-slate-200 border-none text-slate-500 text-[10px]">{subModules.length} 模块</Tag>
                                            ...
                                            <div key={item.id} draggable={userRole === 'team'} onDragStart={(e) => handleModuleDragStart(e, item)} onDragOver={(e) => { e.preventDefault(); setDragOverModuleId(item.id); }} onDragLeave={() => setDragOverModuleId(null)} onDrop={(e) => handleModuleDrop(e, item)} className={`${dragOverModuleId === item.id ? 'drag-over' : ''} rounded-xl transition-all`}>
                                                <Card hoverable className="border border-blue-50 relative" onClick={() => onSelect(item.id)} extra={userRole === 'team' && (
                                                    ...
                                                    <div className="flex justify-between items-center pt-4 border-t">
                                                        ...
                                                        <span className="text-slate-500 text-sm flex items-center gap-2"><FileTextOutlined /> 相关文档 ({item.docs?.length || 0})</span>
                                                    </div>
                                                </Card>
                                            </div>
                                        ))}
...
const DetailView = ({ current, onBack, onEdit, onToggleIssue, onAddIssue, onOpenDocModal, onDeleteDoc, onAddHistory, userRole, feedbackProps, targetAnchor }) => {
    ...
    return (
        <div className="max-w-7xl mx-auto p-6 fade-in">
            <div className="mb-6 flex justify-between"><Button icon={<LeftOutlined />} onClick={onBack} className="font-bold rounded-lg h-10 px-6">返回看板</Button>{userRole === 'team' && <Button icon={<EditOutlined />} onClick={onEdit} type="primary" className="font-bold rounded-lg h-10">更新信息</Button>}</div>
                    ...
                    <Card className="shadow-sm">
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="flex flex-wrap items-center gap-y-2 gap-x-8 text-slate-500 text-sm">
                                ...
                                <div className="flex items-center gap-2"><ToolOutlined /> 响应策略: <span className="text-slate-800 font-bold">{STRATEGY_OPTIONS.find(o => o.value === current.strategyType)?.label}</span></div>
...
const App = () => {
    ...
    const updateSubItem = async (id, field, newData) => {
        if (!user) return;
        await fetch(`${API_BASE}/modules/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ [field]: newData }) });
        fetchData();
    };
    ...
    const saveModule = async (vals) => {
        ...
        try {
            if (selectedId) {
                await fetch(`${API_BASE}/modules/${selectedId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
                message.success('更新成功');
            } else {
                const maxOrder = modules.reduce((max, m) => Math.max(max, m.order || 0), -1);
                await fetch(`${API_BASE}/modules`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...payload, history: [], docs: [], issues: [], order: maxOrder + 1 }) });
                message.success('创建成功');
            }
            setIsEditModalOpen(false);
            fetchData();
        } catch (e) { message.error('错误'); }
    };
        ...
        reader.onload = async (e) => {
            try {
                ...
                for (const row of rows) {
                    if (!row[1]) continue;
                    const map = { '现有产品改造': 'customization', '重新定制开发': 'development', '高保真DEMO': 'demo', '文字说明响应': 'text_description' };
                    let p = row[4];
                    ...
                    await fetch(`${API_BASE}/modules`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ subsystem: row[0] || '未分组', name: String(row[1]), chapter: row[2] || '', strategyType: map[row[3]] || 'development', progress: progressVal, deadline: d, tender: row[6] || '', strategy: row[7] || '', link: row[8] || '', modifiedModules: row[9] || '', status: (progressVal === 100) ? '已完成' : '进行中', order: nextOrder++, issues: [], feedbackGroups: [], docs: [], history: [], updateTime: dayjs().format('YYYY/M/D') }) });
                }
                message.success('导入完成'); setIsImportModalOpen(false); setImportFileList([]); fetchData();
            } catch (err) { message.error('失败'); } finally { setImportLoading(false); }
    ...
    const handleBatchExport = () => {
        ...
        const map = { customization: '现有产品改造', development: '定制', demo: 'DEMO', text_description: '文字说明响应' };
                ...
                {(!isEditingProjectInfo || userRole !== 'team') ? (
                    <div className="fade-in py-2">
                        <div className="flex justify-between items-start mb-6">
                            ...
                            {userRole === 'team' && (
                                <Button icon={<SettingOutlined />} type="primary" ghost onClick={() => { setIsEditingProjectInfo(true); }}>信息维护</Button>
                            )}
                        </div>
                    ...
                    <Form form={infoForm} layout="vertical" onFinish={handleSaveProjectInfo} onValuesChange={onInfoValuesChange} className="fade-in">
                        <Row gutter={24}>
                            ...
                            <Col span={24}><Divider orientation="left" className="m-0 text-slate-400 text-xs">里程碑节点维护</Divider></Col>
                            ...
                            <Col span={24}><Divider orientation="left" className="m-0 text-slate-400 text-xs">试运行详情</Divider></Col>
                            ...
                            <Col span={24}><Divider orientation="left" className="m-0 text-slate-400 text-xs">终验维护</Divider></Col>
                            <Col span={12}><Form.Item name="finalPlanned" label="项目终验 (计划日期)" dependencies={['trialEnd']} rules={[{ validator: async (_, value) => { const end = infoForm.getFieldValue('trialEnd'); if (value && end && value.isBefore(end, 'day')) { return Promise.reject(new Error('终验日期不能早于试运行结束日期')); } } }]}><DatePicker className="w-full" /></Form.Item></Col>
                            <Col span={12}><Form.Item name="finalActual" label="项目终验 (实际日期)" dependencies={['trialEnd']} rules={[{ validator: async (_, value) => { const end = infoForm.getFieldValue('trialEnd'); if (value && end && value.isBefore(end, 'day')) { return Promise.reject(new Error('终验日期不能早于试运行结束日期')); } } }]}><DatePicker className="w-full" /></Form.Item></Col>
                        </Row>
                    </Form>
                )}
            </Modal>
